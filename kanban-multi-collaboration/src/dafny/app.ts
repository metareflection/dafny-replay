...
const App = {
  ...
  // AppCore functions
  MakeClientState: (baseVersion: number, present: DafnyModel, pending: Action[]) => KanbanAppCore.__default.MakeClientState(new BigNumber(baseVersion), present, _dafny.Seq.of(...(pending || []).map((x: any) => actionFromJson(x)))),
  Init: () => KanbanAppCore.__default.Init(),
  InitServerWithModel: (initModel: DafnyModel) => KanbanAppCore.__default.InitServerWithModel(initModel),
  InitClientFromServer: (server: DafnyServerState) => KanbanAppCore.__default.InitClientFromServer(server),
  ClientLocalDispatch: (client: DafnyClientState, action: DafnyAction) => KanbanAppCore.__default.ClientLocalDispatch(client, action),
  HandleRealtimeUpdate: (client: DafnyClientState, serverVersion: number, serverModel: DafnyModel) => KanbanAppCore.__default.HandleRealtimeUpdate(client, new BigNumber(serverVersion), serverModel),
  ClientAcceptReply: (client: DafnyClientState, newVersion: number, newPresent: DafnyModel) => KanbanAppCore.__default.ClientAcceptReply(client, new BigNumber(newVersion), newPresent),
  Sync: (server: DafnyServerState) => KanbanAppCore.__default.Sync(server),
  ServerVersion: (server: DafnyServerState) => toNumber(KanbanAppCore.__default.ServerVersion(server)),
  ServerModel: (server: DafnyServerState) => KanbanAppCore.__default.ServerModel(server),
  AuditLength: (server: DafnyServerState) => toNumber(KanbanAppCore.__default.AuditLength(server)),
  PendingCount: (client: DafnyClientState) => toNumber(KanbanAppCore.__default.PendingCount(client)),
  ClientModel: (client: DafnyClientState) => KanbanAppCore.__default.ClientModel(client),
  ClientVersion: (client: DafnyClientState) => toNumber(KanbanAppCore.__default.ClientVersion(client)),
  IsAccepted: (reply: DafnyReply) => KanbanAppCore.__default.IsAccepted(reply),
  IsRejected: (reply: DafnyReply) => KanbanAppCore.__default.IsRejected(reply),
