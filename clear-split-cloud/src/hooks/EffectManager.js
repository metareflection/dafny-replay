...
export class EffectManager {
    ...
    if (App.EffectCommand.isSendDispatch(command)) {
      ...
      try {
        ...
        if (data.status === 'accepted') {
          ...
          const fresh = await backend.groups.load(this.#groupId)
  ...
  async start() {
    ...
    try {
      const { state, version } = await backend.groups.load(this.#groupId)
  ...
  #subscribeRealtime() {
    this.#realtimeUnsubscribe = backend.realtime.subscribe(this.#groupId, (version, state) => {
